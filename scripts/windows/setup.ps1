...
# setup.ps1 - build environment setup for a project whose build\ holds this toolchain (Windows / PowerShell).
...
# - Clones Arduino libraries from build/required_libraries.txt into build/libraries (created if missing)
...
$RequirementsFile = Join-Path $BuildRoot 'required_libraries.txt'
...
function Init-RequiredLibraries {
  if (Test-Path $RequirementsFile) { Write-Ok "Libraries list already exists: $RequirementsFile"; return }
  @'
...
'@ | Set-Content -LiteralPath $RequirementsFile -Encoding ASCII
  Write-Ok "Created libraries list: $RequirementsFile"
}
...
c6,
s3,
'@ | Set-Content -LiteralPath $ReleaseMatrix -Encoding ASCII
...
function Ensure-Gitignore {
  ...
  $required = @('scripts/', 'tools/', '.venv/', 'builds/', 'libraries/', 'build_config', 'build_config.ps1')
